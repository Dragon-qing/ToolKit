...
void BTFProcessThread::run()
{
    ...
    {
        Logger::Instance().AddLog(CRITICAL_LOG, TR("打包失败，文件列表为空或保存名字为空"));
        return;
    }
    ...
    if (res.contains("Everything is Ok"))
    {
        ...
        runLogProcess.write("exit\n");
        runLogProcess.waitForFinished(-1);
        Logger::Instance().AddLog(INFO_LOG, TR("%1已完成BTF打包").arg(m_sSaveName));
        emit MakeDone();
    }
    else
    {
        Logger::Instance().AddLog(CRITICAL_LOG, res);
        emit MakeFaild();
    }
    runLogProcess.close();
}
